...
```{r 4.2.1 Add WIC Property Lines in MARS DB, eval= params$write == TRUE & nrow(new_wic_propertyline_sf) > 0, include=FALSE}
  ...
  # add new wic propertylines
   success_property_sf <- st_write(obj = new_wic_propertyline_sf, dsn = mars_con, Id(schema="fieldwork", table = "beta_tbl_wic_propertyline_geom"), append = TRUE)
...
```{r 4.2.2 Add WIC Footprints in MARS DB, eval= params$write == TRUE & nrow(new_wic_footprint_sf) > 0, include=FALSE}
  ...
  # add new wic footprints
   success_footprint_sf <- st_write(obj = new_wic_footprint_sf, dsn = mars_con, Id(schema="fieldwork", table = "beta_tbl_wic_footprint_geom"), append = TRUE)
...
```{r 5.3 Identify New/Change SMP-WIC, include=FALSE}
  mars_wic_system <- dbGetQuery(mars_con, "SELECT * FROM fieldwork.beta_tbl_wic_system")
  # updating tables with the system ids and workorder ids for status record keeping
  mars_wic_system_status <- dbGetQuery(mars_con, "SELECT * FROM fieldwork.beta_tbl_wic_system_status")
  mars_wic_wo_status <- dbGetQuery(mars_con, "SELECT * FROM fieldwork.beta_tbl_wic_wo_status")
...
```{r 5.4 Add New System-WIC to MARS DB, eval= params$write == TRUE & nrow(new_wic_system) > 0, include=FALSE}
  # add new wic-smp
   success_wic_system <- dbWriteTable(mars_con, Id(schema = "fieldwork", table = "beta_tbl_wic_system"), new_wic_system, append= TRUE, row.names = FALSE )
...
```{r Update Changed SMP-WIC in MARS DB, eval= params$write == TRUE & nrow(changed_wic_system) > 0, include=FALSE}
  # update changed wic-smp
   for (i in 1:nrow(changed_wic_system)) {
      ...
      "UPDATE fieldwork.beta_tbl_wic_system
...
```{r 5.6 Add New  Unique Systems to MARS DB for Status Record Keeping, eval= params$write == TRUE & nrow(new_wic_system_status) > 0, include=FALSE}
  # add new wic-smp
   success_wic_system_status <- dbWriteTable(mars_con, Id(schema = "fieldwork", table = "beta_tbl_wic_system_status"), new_wic_system_status, append= TRUE, row.names = FALSE )
...
```{r 5.7 Add New  Unique Workorder ids to MARS DB for Status Record Keeping, eval= params$write == TRUE & nrow(new_wic_wo_status) > 0, include=FALSE}
  # add new wic-smp
   success_wic_wo_status <- dbWriteTable(mars_con, Id(schema = "fieldwork", table = "beta_tbl_wic_wo_status"), new_wic_wo_status, append= TRUE, row.names = FALSE )
