...
```{r 1. Prepping the data for storage in MARS DB (renaming, deduplicating, type cast etc), include=FALSE}
 ...
 # de-duplicate workorders
   ...
   # Use sub() to remove everything up to and including the first occurrence of "Comments:". this is to make comments more readable
   cw_comments$comment <- sub(".*?Comments: ", "", cw_comments$comment)
...
```{r 3.5 Writing new wic parcels to MARS DB , eval= params$write == TRUE & nrow(new_wic_parcel) > 0, include=FALSE}
  # add new wic parcels
   success_wicparcels <- dbWriteTable(mars_con, Id(schema = "fieldwork", table = "tbl_wic_parcel"), new_wic_parcel, append= TRUE, row.names = FALSE )
...
```{r 4.2.1 Add WIC Property Lines in MARS DB, eval= params$write == TRUE & nrow(new_wic_propertyline_sf) > 0, include=FALSE}
  ...
  # add new wic propertylines
   success_property_sf <- st_write(obj = new_wic_propertyline_sf, dsn = mars_con, Id(schema="fieldwork", table = "tbl_wic_propertyline_geom"), append = TRUE)
...
```{r 4.2.2 Add WIC Footprints in MARS DB, eval= params$write == TRUE & nrow(new_wic_footprint_sf) > 0, include=FALSE}
  ...
  # add new wic footprints
   success_footprint_sf <- st_write(obj = new_wic_footprint_sf, dsn = mars_con, Id(schema="fieldwork", table = "tbl_wic_footprint_geom"), append = TRUE)
...
```{r 4.4 New SMP Polygons WKTs in MARS DB, eval= params$write == TRUE & nrow(new_smp_sf) > 0, include=FALSE}
  # add new smp polygons with postgis
   success_smppoly <- st_write(obj = new_smp_sf, dsn = mars_con, Id(schema="fieldwork", table = "tbl_wic_smp_geom"), append = TRUE)
...
```{r 5.1 Intersect WIC Parcels with System Polygons and Calculate Distance, include=FALSE}
...
```{r 5.4 Add New System-WIC to MARS DB, eval= params$write == TRUE & nrow(new_wic_system) > 0, include=FALSE}
  # add new wic-smp
   success_wic_system <- dbWriteTable(mars_con, Id(schema = "fieldwork", table = "tbl_wic_system"), new_wic_system, append= TRUE, row.names = FALSE )

```

## 5.5 Update Changed SMP-WIC in MARS DB {.hidden .unlisted}

```{r Update Changed SMP-WIC in MARS DB, eval= params$write == TRUE & nrow(changed_wic_system) > 0, include=FALSE}
...
```{r 5.6 Add New  Unique Systems to MARS DB for Status Record Keeping, eval= params$write == TRUE & nrow(new_wic_system_status) > 0, include=FALSE}
  # add new wic-smp
   success_wic_system_status <- dbWriteTable(mars_con, Id(schema = "fieldwork", table = "tbl_wic_system_status"), new_wic_system_status, append= TRUE, row.names = FALSE )

```

## 5.7 Add New Unique Workorder ids to MARS DB for Status Record Keeping {.hidden .unlisted}

```{r 5.7 Add New  Unique Workorder ids to MARS DB for Status Record Keeping, eval= params$write == TRUE & nrow(new_wic_wo_status) > 0, include=FALSE}
  # add new wic-smp
   success_wic_wo_status <- dbWriteTable(mars_con, Id(schema = "fieldwork", table = "tbl_wic_wo_status"), new_wic_wo_status, append= TRUE, row.names = FALSE )
