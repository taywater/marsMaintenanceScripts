...
```{r 5.1 Intersect WIC Parcels with System Polygons and Calculate Distance, include=FALSE}
  ...
  # placeholder
  smp_wic_df <- data.frame(system_id = as.character(),
  ...
  for (i in 1:length(system_wic_intersect)) {
    if (length(system_wic_intersect[[i]]) > 0) {
      for (j in 1:length(system_wic_intersect[[i]])) {
        ...
        # property distance
        temp_smp_wic_prop_dist <- st_distance(system_sf[i, ], wic_propertyline_combined_sf[wic_sf_index[j], ])
        # footprint distance
        temp_smp_wic_footprint_dist <- st_distance(system_sf[i, ], filter(wic_footprint_combined_sf, address == wic_propertyline_combined_sf[wic_sf_index[j], ]$address))
        ...
        temp_smp_wic_df <- data.frame(system_id = intersecting_systems[i, ], 
...
```{r 5.2 Identify the WIC Construction Status, include=FALSE}
  ...
  smp_milestones <- smpbdv %>%
    ...
    inner_join(smp_wic_df, by = "system_id") %>%
...
## 5.3 Identify New/Change WIC near Systems
...
## 5.6 Summary Table of New/Changed WICs within Systems

### 5.6.1 Table 1. New WICs around Systems (100 ft buffer)

```{r Section 5.6.1- Summary Table of New WICs around SMPs, echo=FALSE, results='asis', eval= nrow(new_wic_system)>0}
      # Prep the table
...
### 5.6.2 Table 2. Changed WICs around Systems (100 ft buffer)

```{r Section 5.6.2 - Summary Table of Changed WICs around SMPs, echo=FALSE, results='asis', eval= nrow(changed_wic_system)>0}
       # Prep the table
