...
```{r 5.2 Identify the WIC Construction Status, include=FALSE}
  ...
  smp_milestones <- smpbdv %>%
    ...
    select (smp_id, workorder_id, wic_address, date, property_dist_ft, footprint_dist_ft, construction_start_date, pc_ntp_date, construction_complete_date, contract_closed_date) %>%
    distinct()
  ...
  wic_smp <- smp_milestones %>%
    select(smp_id, workorder_id, wic_address, con_phase_lookup_uid, footprint_dist_ft, property_dist_ft) %>%
    distinct()
  
```

## 5.3 Identify New/Change WIC near SMPs
...
```{r 5.3 Identify New/Change SMP-WIC, include=FALSE}
  ...
  changed_wic_smp <- wic_smp %>%
     ...
     anti_join(mars_wic_smp, by = c("smp_id", "workorder_id", "wic_address", "con_phase_lookup_uid", "footprint_dist_ft", "property_dist_ft")) 
```

Number of New WICs within 100 ft of SMPs: `r colorize(nrow(new_wic_smp),"red")`

Number of Changed WICs within 100 ft of SMPs: `r colorize(nrow(changed_wic_smp), "red")`
   ...
   for (i in 1:nrow(changed_wic_smp)) {
      ...
      "UPDATE fieldwork.beta_tbl_wic_smp
       SET con_phase_lookup_uid = '%s',
           ...
           property_dist_ft = '%s',
           wic_address = '%s'
       WHERE workorder_id = %s AND smp_id = '%s';",
      ...
      changed_wic_smp$workorder_id[i],
...
## 5.6 Summary Table of New/Changed WICs within SMPs

### 5.6.1 Table 1. New WICs around SMPs (100 ft buffer)

```{r Section 5.6.1- Summary Table of New WICs around SMPs, echo=FALSE, results='asis', eval= nrow(new_wic_smp)>0}
      # round digits
      new_wic_smp[,"footprint_dist_ft"] <- format(round(new_wic_smp[,"footprint_dist_ft"], 2), nsmall = 2)
      new_wic_smp[,"property_dist_ft"] <- format(round(new_wic_smp[,"property_dist_ft"], 2), nsmall = 2)
      
      # Prep the table
      new_wic_kable <- new_wic_smp %>%
        inner_join(cw_workorders, by = "workorder_id") %>%
        select(`SMP ID` = smp_id, `Work Order ID` = workorder_id , Date = date,`WIC Address` = wic_address, `Dist. Footprint (ft)` = footprint_dist_ft, `Dist. Property (ft)` = property_dist_ft) %>%
        arrange(desc(Date)) %>%
        distinct()
...
### 5.6.2 Table 2. Changed WICs around SMPs (100 ft buffer)

```{r Section 5.6.2 - Summary Table of Changed WICs around SMPs, echo=FALSE, results='asis', eval= nrow(changed_wic_smp)>0}
       # round digits
      changed_wic_smp[,"footprint_dist_ft"] <- format(round(changed_wic_smp[,"footprint_dist_ft"], 2), nsmall = 2)
      changed_wic_smp[,"property_dist_ft"] <- format(round(changed_wic_smp[,"property_dist_ft"], 2), nsmall = 2)
