...
```{r 1. Prepping the data for storage in MARS DB (renaming, deduplicating, type cast etc)}
 # cityworks wic orders and comments tables
   ...
   (wo.DESCRIPTION = 'A - LEAK INVESTIGATION' AND r.DESCRIPTION = 'WATER IN CELLAR'))
      ")
   cw_comments <- dbGetQuery(cityworks_con, "SELECT WORKORDERID, COMMENTS from Azteca.WORKORDERCOMMENT")

 # Make names & data format consistent 
   names(cw_workorders) <- c("workorder_id","date","address","xcoordinate","ycoordinate","facility_id")
   names(cw_comments) <- c("workorder_id","comment")
 # Remove {} from facility ids
   #cw_workorders$facility_id <- gsub("\\{(.*)\\}","\\1",as.character(cw_workorders$facility_id))
...
```{r 2.1 Checking for new WIC complaints, new WIC comments and changed ones}
 ...
 # Changed WICs
   changed_wic <- cw_workorders %>%
     ...
     anti_join(mars_workorders, by = c("date","address","xcoordinate","ycoordinate","facility_id"))
   
 # Combined new and changed WIC dataframe
   intersecting_wics <- bind_rows(new_wic, changed_wic)
   
 # New Comments
   new_comment <- cw_comments %>%
     filter(workorder_id %!in% mars_comments$workorder_id)
 ...
 # Changed Comments
   changed_comment <- cw_comments %>%
     filter(workorder_id %!in% new_comment$workorder_id) %>%
     anti_join(mars_comments, by = "comment")
...
```{r 2.5 Updating changed comments to MARS DB, eval= params$write == TRUE & nrow(changed_comment) > 0}
 ...
 # update changed comments
   upcomm_sql_string <- "UPDATE fieldwork.beta_tbl_wic_comment set comment = '%s' WHERE workorder_id = %s;"
   success_upcomments <- dbSendStatement(mars_con, paste(sprintf(upcomm_sql_string, changed_comment$comment, changed_comment$workorder_id), collapse=""))
...
```{r 3.4 Binding the WIC parcels, pulling wic parcels from MARS DB and compare, eval= nrow(wicparcel_xy) > 0 | nrow(wicparcel_join_facilityid) > 0 | nrow(wicparcel_join_address) > 0}
 ...
 # changed wic parcel
  changed_wic_parcel <- wic_parcels %>%
    ...
    anti_join(mars_wic_parcels, by = c("address", "facility_id"))
...
# 4. Add/Update WKT Polygons

## 4.1 Identify New/Changed WIC Polygons (Property & Footprint)

```{r 4.1 Identifying New/Changed WIC Polygons}
 # pull mars polygons
 mars_nonsmp_polygon <- dbGetQuery(mars_con, "SELECT * FROM fieldwork.beta_tbl_wic_nonsmp_polygon")
 ...
 wic_propertyline_sf <- pwd_parcels_sf %>%
   ...
   select(address = gis_address)
 
 # convert footprint sf to wkt 
 footprint_address <- wic_footprint_sf %>%
   st_transform(4326) %>%
   st_set_geometry(NULL)
 
 footprint_wkt <- wic_footprint_sf %>%
    st_transform(4326) %>%
    st_geometry() %>%
    st_as_text()
 
 # convert property sf to wkt 
 propertyline_address <- wic_propertyline_sf %>%
   st_transform(4326) %>%
   st_set_geometry(NULL)
 
 propertyline_wkt <- wic_propertyline_sf %>%
    st_transform(4326) %>%
    st_geometry() %>%
    st_as_text()
 
 # storing the results in dataframe
 footprint_wkt_df <- data.frame(address = footprint_address, footprint_wkt = footprint_wkt) %>%
   distinct()
 property_wkt_df <- data.frame(address = propertyline_address, propertyline_wkt = propertyline_wkt) %>%
   distinct()
 wic_polygon_df <- footprint_wkt_df %>%
   full_join(property_wkt_df, by = "address")
 
 # new Polygons
 new_wic_polygons <- wic_polygon_df %>%
   filter(address %!in% mars_nonsmp_polygon$address)
 # changed Polygons
 changed_wic_polygons <- wic_polygon_df %>%
   filter(address %!in% new_wic_polygons$address) %>%
   anti_join(mars_nonsmp_polygon, by = c("footprint_wkt","propertyline_wkt"))

```

## 4.2 Add WIC Polygons to MARS DB

```{r 4.2 Add WIC Polygons WKTs in MARS DB, eval= params$write == TRUE & nrow(new_wic_polygons) > 0}

  # add new wic polygons
   success_wicpoly <- dbWriteTable(mars_con, Id(schema = "fieldwork", table = "beta_tbl_wic_nonsmp_polygon"), new_wic_polygons, append= TRUE, row.names = FALSE )

```

## 4.3 Update WIC Polygons in MARS DB

```{r 4.3 Updating WIC Polygons WKTs in MARS DB, eval= params$write == TRUE & nrow(changed_wic_polygons) > 0}

  # update changed wic polygons
   upwicpoly_sql_string <- "UPDATE fieldwork.beta_tbl_wic_nonsmp_polygon set propertyline_wkt = '%s', footprint_wkt = '%s' WHERE address = '%s';"
   success_upwicpoly <- dbSendStatement(mars_con, paste(sprintf(upwicpoly_sql_string, changed_wic_polygons$propertyline_wkt, changed_wic_polygons$footprint_wkt, changed_wic_polygons$address), collapse=""))

```

## 4.4 Identify New/Changed SMP Polygons

```{r 4.4 Identify New/Changed SMP polygons}
  # pull mars polygons
  mars_smp_polygon <- dbGetQuery(mars_con, "SELECT * FROM fieldwork.beta_tbl_wic_smp_polygon")
  ...
  # Storing the results in dataframe
  smp_wkt_df <- data.frame(smp_id = smp_id_col, smp_wkt = smp_wkt) %>%
     distinct()

  # New Polygons
  new_smp_polygons <- smp_wkt_df %>%
     filter(smp_id %!in% mars_smp_polygon$smp_id)
  # Changed Polygons
  changed_smp_polygons <- smp_wkt_df %>%
     filter(smp_id %!in% new_smp_polygons$smp_id) %>%
     anti_join(mars_smp_polygon, by = "smp_wkt")   
  
```

## 4.5 Add New SMP Polygons to MARS DB

```{r 4.5 New SMP Polygons WKTs in MARS DB, eval= params$write == TRUE & nrow(new_smp_polygons) > 0}

  # add new wic polygons
   success_smppoly <- dbWriteTable(mars_con, Id(schema = "fieldwork", table = "beta_tbl_wic_smp_polygon"), new_smp_polygons, append= TRUE, row.names = FALSE )

```

## 4.6 Update New SMP Polygons in MARS DB

```{r 4.6 Update New SMP Polygons WKTs in MARS DB, eval= params$write == TRUE & nrow(changed_smp_polygons) > 0}

  # update changed wic polygons
   upsmppoly_sql_string <- "UPDATE fieldwork.beta_tbl_wic_smp_polygon set smp_wkt = '%s' WHERE smp_id = '%s';"
   success_upsmppoly <- dbSendStatement(mars_con, paste(sprintf(upsmppoly_sql_string, changed_smp_polygons$smp_wkt, changed_smp_polygons$smp_id), collapse=""))
...
```{r 5.1 Intersect WIC Parcels with SMP Polygons and Calculate Distance}
  ...
  smp_wic_intersect <- st_intersects(smp_sf_100ft, wic_propertyline_sf)
  
  intersecting_addresses <- wic_propertyline_sf %>%
    st_set_geometry(NULL)
  ...
  for (i in 1:length(smp_wic_intersect)) {
    if (length(smp_wic_intersect[[i]]) > 0) {
      for (j in 1:length(smp_wic_intersect[[i]])) {
        ...
        # property distance
        temp_smp_wic_prop_dist <- st_distance(smp_sf[i, ], wic_propertyline_sf[wic_sf_index[j], ])
        # footprint distance
        temp_smp_wic_footprint_dist <- st_distance(smp_sf[i, ], filter(wic_footprint_sf, address == wic_propertyline_sf[wic_sf_index[j], ]$address))
        ...
        temp_smp_wic_df <- data.frame(smp_id = intersecting_smps[i, ], 
                                      wic_address = intersecting_addresses[smp_wic_intersect[[i]],],
...
```{r 5.2 Identify the WIC Construction Status}
  ...
  smp_milestones <- smpbdv %>%
    ...
    select (smp_id, workorder_id, date, property_dist_ft, footprint_dist_ft, construction_start_date, pc_ntp_date, construction_complete_date, contract_closed_date) %>%
    distinct()
  ...
  wic_smp <- smp_milestones %>%
    select(smp_id, workorder_id, con_phase_lookup_uid, footprint_dist_ft, property_dist_ft) %>%
    distinct()
...
```{r 5.3 Identify New/Change SMP-WIC}
  ...
  # New wic_smp
  new_wic_smp <- wic_smp %>%
     anti_join(mars_wic_smp, by = c("smp_id","workorder_id"))
  
  # Changed wic_smp
  changed_wic_smp <- wic_smp %>%
     anti_join(new_wic_smp, by = c("smp_id","workorder_id")) %>%
     anti_join(mars_wic_smp, by = c("con_phase_lookup_uid", "footprint_dist_ft", "property_dist_ft"))  
...
```{r Update Changed SMP-WIC in MARS DB, eval= params$write == TRUE & nrow(changed_wic_smp) > 0}
  # update changed wic-smp
   upwicsmp_sql_string <- "UPDATE fieldwork.beta_tbl_wic_smp set con_phase_lookup_uid = %s, footprint_dist_ft = %s, property_dist_ft = %s WHERE workorder_id = %s AND smp_id = '%s;"
   success_upwicsmp <- dbSendStatement(mars_con, paste(sprintf(upwicsmp_sql_string, changed_wic_smp$con_phase_lookup_uid, changed_wic_smp$footprint_dist_ft, changed_wic_smp$property_dist_ft, changed_wic_smp$workorder_id, changed_wic_smp$smp_id), collapse=""))
