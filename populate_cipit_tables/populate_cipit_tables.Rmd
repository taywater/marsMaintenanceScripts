...
tryCatch(
  ...
  expr = {
    update_cipit_project <- dbSendQuery(mars, 'update external.tbl_cipit_project set work_number=?, project_phase=?, project_phase_status=?, project_title=?, targeted_bid_fy=?, pc_ntp_date=?, construction_start_date=?, design_engineer=?, division_engineer=?, contractor=?, projectautoid=?, construction_complete_date=?, contract_closed_date=?, md5hash=? where cipit_project_uid=?')
