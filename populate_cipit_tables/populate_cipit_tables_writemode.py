...
r_script = "'" + re.sub('\\\\', '\\\\\\\\',MAINTENANCEFOLDER) + "\\\\populate_cipit_tables\\\\populate_cipit_tables.rmd'"
database = "'mars_data'"
writeflag = "TRUE"
output_file = "'output\\\\" + datestring + "populate_cipit_tables.html" + "'"
...
outputexists = os.path.isfile(MAINTENANCEFOLDER + "\\populate_cipit_tables\\" + re.sub('\\\\\\\\', '\\\\', output_file.strip("'")))
if outputexists:
	webbrowser.open(os.path.realpath(MAINTENANCEFOLDER + "\\populate_cipit_tables\\" + re.sub('\\\\\\\\', '\\\\', output_file.strip("'"))))
else:
	webbrowser.open(os.path.realpath(MAINTENANCEFOLDER + "\\populate_cipit_tables\\cipit_error.html"))