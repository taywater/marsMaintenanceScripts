...
if(!kill){
  new_records <- rbind(new_smps, new_systems, new_projects) %>% nrow
  logMessage <- data.frame(date = as.Date(today()),
                     records = new_records,
                     type = "Records",
                     hash = log_code)

  dbWriteTable(marsDBCon, DBI::SQL("log.tbl_writes_greenit"), logMessage, append = TRUE, row.names=FALSE)
}
...
if(!kill){
  updated_records <- rbind(update_smps, update_systems, update_projects) %>% nrow
  logMessage <- data.frame(date = as.Date(today()),
