...
```{r Filter for new projects, echo = FALSE, include = FALSE, eval = keepRunning}
...
```{r Section We Are As One - external.tbl_smpbdv, echo = FALSE, eval = all(keepRunning, newAssets, nrow(new_smps) > 0)}
###New SMP Polygons
knitr::asis_output(paste("### New SMPs to add to `external.tbl_smpbdv`: ",   nrow(new_smps)))
kable(head(new_smps))
```

```{r Section We Are As One - external.tbl_systembdv, echo = FALSE, eval = all(keepRunning, newAssets, nrow(new_systems) > 0)}
knitr::asis_output(paste("### New systems to add to `external.tbl_systembdv`: ",   nrow(new_systems)))
kable(head(new_systems))
```

```{r Section We Are As One - external.tbl_projectbdv, echo = FALSE, eval = all(keepRunning, newAssets, nrow(new_projects) > 0)}
...
```{r Section We Are As One - Updates to external.tbl_gswibasin, echo = FALSE, include = TRUE, eval = all(keepRunning, newUpdates, nrow(update_smps) > 0)}
knitr::asis_output(paste("### SMPs to update in `external.tbl_smpbdv`: ",   nrow(update_smps)))
kable(head(update_smps))
```

```{r Section We Are As One - Updates to external.tbl_gswiblueroof, echo = FALSE, include = TRUE, eval = all(keepRunning, newUpdates, nrow(update_systems) > 0)}
knitr::asis_output(paste("### Systems to update in `external.tbl_systembdv`: ",   nrow(update_systems)))
kable(head(update_systems))
```

```{r Section We Are As One - Updates to external.tbl_gswibumpout, echo = FALSE, include = TRUE, eval = all(keepRunning, newUpdates, nrow(update_projects) > 0)}
