...
```{r Section 1 - Gathering baro data, include = FALSE}
  ...
  baro_rawfolders <- dbGetQuery(marsDBCon, "SELECT * FROM admin.baro_rawfolder")

  #Fetch all distinct baro_rawfile_uids from the baro table, to guard against files existing in baro_rawfile but not in baro
  fileswithdata <- dbGetQuery(marsDBCon, "SELECT baro_rawfile_uid, filepath from admin.baro_rawfile where baro_rawfile_uid in (select distinct baro_rawfile_uid from data.baro)")
  
  #Fetch all of the latest dates for every baro in the baro table
  latestdates <- dbGetQuery(marsDBCon, "SELECT r.smp_id, max(b.dtime_est) as latestdate_est FROM data.baro b LEFT JOIN admin.baro_rawfile r ON b.baro_rawfile_uid = r.baro_rawfile_uid GROUP BY smp_id")
...
## Updates to `admin.baro_rawfile` Table
...
```{r Section 2 - Gathering the baro data, include = FALSE}
  ...
  existingfiletable <- dbGetQuery(marsDBCon, "SELECT * FROM admin.baro_rawfile")
