...
```{r Section 1 - Gathering baro data, include = FALSE, eval = keepRunning}
...
```{r Section R1: New Files Output, echo = FALSE, eval = all(keepRunning, nrow(display_filetable) > 0)}
    knitr::asis_output(paste("### New Baro Files to add to `admin.tbl_baro_rawfile`: ",  nrow(display_filetable)))
...
```{r Section 2 - Gathering the baro data, include = FALSE, eval = keepRunning}
...
```{r Section 2.5, Writing to the baro table, include = FALSE, eval = keepRunning}
  
  knitr::asis_output(paste0("###The ", nrow(output_newfiles), " new files contain ", nrow(newdata), " new data points"))
