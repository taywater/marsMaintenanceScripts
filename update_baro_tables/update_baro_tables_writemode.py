...
r_script = "'" + re.sub('\\\\', '\\\\\\\\',MAINTENANCEFOLDER) + "_pg9\\\\update_baro_tables\\\\update_baro_tables.rmd'"
database = "'mars_testing'"
writeflag = "TRUE"
output_file = "'output\\\\" + datestring + "_update_baro_tables.html" + "'"
...
outputexists = os.path.isfile(MAINTENANCEFOLDER + "_pg9\\update_baro_tables\\" + re.sub('\\\\\\\\', '\\\\', output_file.strip("'")))
if outputexists:
	webbrowser.open(os.path.realpath(MAINTENANCEFOLDER + "_pg9\\update_baro_tables\\" + re.sub('\\\\\\\\', '\\\\', output_file.strip("'"))))
else:
	webbrowser.open(os.path.realpath(MAINTENANCEFOLDER + "_pg9\\update_baro_tables\\baro_error.html"))
