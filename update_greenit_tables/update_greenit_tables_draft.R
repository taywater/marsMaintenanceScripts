...
#1.3 initial WRITE ------
#smp
#dbWriteTable(mars_data, DBI::SQL("foreign.smpbdv"), smp_hash, append= TRUE)

#system
#dbWriteTable(mars_data, DBI::SQL("foreign.systembdv"), system_hash, append = TRUE)

#project
#dbWriteTable(mars_data, DBI::SQL("foreign.projectbdv"), project_hash, append = TRUE)
...
smp_md <- dbGetQuery(mars_data, "select * from foreign.smpbdv")

system_md <- dbGetQuery(mars_data, "select * from foreign.systembdv")

project_md <- dbGetQuery(mars_data, "select * from foreign.projectbdv")
...
# #smp
# dbWriteTable(mars_data, DBI::SQL("foreign.smpbdv"), new_smps, append= TRUE)
# 
# #system
# dbWriteTable(mars_data, DBI::SQL("foreign.systembdv"), new_systems, append = TRUE)
# 
# #project
# dbWriteTable(mars_data, DBI::SQL("foreign.projectbdv"), new_projects, append = TRUE)
...
#2.6 compare and find new HASHES based on smp, system, and projects -----

#smp
new_smp_hashes <- smp_hash %>% left_join(smp_md_trim, by = "smp_id") %>% 
  anti_join(smp_md, by = c("smp_id", "hash_md5"))

#system
new_system_hashes <- system_hash %>% anti_join(system_md, by = c("system_id", "hash_md5"))

#project
new_project_hashes <- project_hash %>% anti_join(project_md, by = c("project_id", "hash_md5"))

#2.7 write/update new stuff 
#2.7.1 SMP -----
#write update query
update_smp <- dbSendQuery(mars_data, 'update foreign.smpbdv set project_id=?, smp_dataphase=?, worknumber=?, capit_status=?, capit_statuscategory=?, system_id=?, smp_id=?, smp_smptype=?, smp_footprint_ft2=?, smp_smptrees=?, smp_perviousarea_ft2=?, smp_vegetatedarea_ft2=?, smp_storagedepth_ft=?, smp_pondingdepth_in=?, smp_storagetype=?, smp_pretreatment=?, smp_notbuiltretired=?, smp_notbuiltretiredreason=?, hash_md5=? WHERE smpbdv_uid=?')
