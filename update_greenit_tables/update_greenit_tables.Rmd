...
```{r Section 2: Join Tables and Write New Data to Postgres, echo = FALSE}
...
#### The following `r sum(nrow(new_smps),nrow(new_systems),nrow(new_projects))` records have `r if(!params$write){"not"}` been added to the `external.smpbdv`, `external.systemsbdv`, and `external.projectbdv` tables within `mars_data`:
`r if(nrow(new_smps)>0){knitr::kable(new_smps, caption = "SMPs")}`
`r if(nrow(new_systems)>0){knitr::kable(new_systems, caption = "Systems")}`
`r if(nrow(new_projects)>0){knitr::kable(new_projects, caption = "Projects")}`

```{r Section 3: Rerun Query and Update Data in Postgres, echo = FALSE}

#3.1 Read Postgres with new records added
...
#3.4 Send Updates if write == TRUE
if(params$write){
...
```
#### The following `r sum(nrow(new_smp_hashes),nrow(new_system_hashes),nrow(new_project_hashes))` records have `r if(!params$write){"not"}` been upadated in the `external.smpbdv`, `external.systemsbdv`, and `external.projectbdv` tables within `mars_data`:
`r if(nrow(new_smp_hashes)>0){knitr::kable(new_smp_hashes, caption = "SMPs")}`
`r if(nrow(new_system_hashes)>0){knitr::kable(new_system_hashes, caption = "Systems")}`
`r if(nrow(new_project_hashes)>0){knitr::kable(new_project_hashes, caption = "Projects")}`
