...
--         ON UPDATE CASCADE
--         ON DELETE RESTRICT,
--          add a constraint here for research_lookup_uid_fkey
...
--  j.	srt (updated 20210824)
CREATE TABLE fieldwork.srt
(
    srt_uid integer NOT NULL DEFAULT nextval('fieldwork.srt_srt_uid_seq'::regclass),
    system_id text COLLATE pg_catalog."default" NOT NULL,
    test_date timestamp without time zone NOT NULL,
    con_phase_lookup_uid integer NOT NULL,
    ...
    flow_data_recorded boolean,
    water_level_recorded boolean,
    photos_uploaded boolean,
    sensor_collection_date timestamp without time zone,
    qaqc_complete boolean,
    srt_summary_date timestamp without time zone,
    sensor_deployed boolean,
    CONSTRAINT srt_pkey PRIMARY KEY (srt_uid),
    CONSTRAINT system_validity CHECK (admin.system_exists(system_id) = true) NOT VALID
)
...
(
    ...
    draindown_assessment_lookup_uid integer NOT NULL,
