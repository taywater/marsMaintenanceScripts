...
```{r Section 1A - Gathering OW data, include = FALSE}
  ...
  ow_latestdates <- dbGetQuery(marsDBCon, "SELECT * FROM data.owdata_latestdates") %>%
  ...
  accessdb <- dbGetQuery(marsDBCon, "SELECT filepath, ow_uid, datatable, sumptable FROM admin.accessdb")
...
## Updates to `data.ow_leveldata_raw` Table

The Access DBs for each observation well have been examined. There are `r nrow(newdata)` new measurements to add to the `data.ow_leveldata_raw` table.
...
```{r Section 2A - Gathering GW data, include = FALSE}
  ...
  #Read the latest date from each observation well from the mars database
  gw_latestdates <- dbGetQuery(marsDBCon, "SELECT * FROM data.gwdata_latestdates") %>%
    mutate(dtime_est = force_tz(dtime_est, tz = "EST")) #Adjust time zone without moving timestamp
...
## Updates to `data.gw_leveldata_raw` Table
