...
```{r Section 1B - Writing to the ow_leveldata table, include = FALSE, eval = all(keepRunning, nrow(display_newdata) > 0)}	
  ###This is from the baro script. Fix this
  knitr::asis_output(paste0("###The ", nrow(display_newdata), " updated wells contain ", nrow(newdata), " new data points"))
  ...
  for(i in 1:nrow(newdata_results)){
    if(nrow(newdata_results) == 0){break} #If there are no new data sources, don't do anything
