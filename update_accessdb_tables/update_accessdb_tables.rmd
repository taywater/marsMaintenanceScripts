...
This script is attempting to update the `accessdb` and `ow_sumpdepth`tables in the `r params$database` database. It crawls the GSI Monitoring Sites folder, and does the following:  
  ...
  * For newly created Access DBs, parse their filepath and look in `ow` for matching `ow.smp_id` and `ow.ow_suffix`
 ...
 3. Update the sump depths in the `ow_sumpdepth` table
...
```{r Section 1 - Gathering data for the accessdb table, include = FALSE}
  ...
  ###Section 1.3 Connect ow_uids to SMP IDs and OW suffixes
    ...
    ow <- dbGetQuery(marsDBCon, "SELECT * FROM fieldwork_ow")
  ...
  ###Section 1.4 Reconcile the new Access DBs with the existing defined OWs
    ...
    #1.4.3 Defined OWs that do not have Access DBs created
    owswithoutaccessdb <- filter(ow, !(ow_uid %in% accessdbtable_server$ow_uid | ow_uid %in% appendtoaccessdbtable$ow_uid))
    
```


## State of site databases relative to `accessdb` and `ow`

The site folders have been crawled and compared to the `accessdb` table and `ow` table. The following summaries show the reconciliations and updates that have been/must be made.  
  
### New Access DBs to add to `accessdb`
  
There are `r nrow(appendtoaccessdbtable)` new Access DBs to add to the `accessdb` table. These DBs have defined OWs in the `ow` table.
...
```{r Section 1.5 - Writing to the accessdb table, include = FALSE}	
  ...
  #If there's new data to append
    ...
    tryCatch({outcome_appendtoaccessdbtable <- dbWriteTable(marsDBCon, 
        "accessdb", 
...
### Access DBs without a defined Observation Well in `ow`  
The script found `r nrow(accessdbswithoutow)` Access DBs without corresponding observation wells defined in `ow`. `r if(nrow(accessdbswithoutow) > 0) "Please create observation wells for these Access DBs with the \"Add Observation Well\" form in the MARS Fieldwork Tracking Database."`  
...
```{r Section 3 - Gather data for new/updated sump depths, include=FALSE}
  ...
  accessdbs <- dbGetQuery(marsDBCon, "SELECT * FROM accessdb")
  ow <- dbGetQuery(marsDBCon, "SELECT * FROM fieldwork_ow")
  ow_sumpdepth <- dbGetQuery(marsDBCon, "SELECT * FROM ow_sumpdepth")
  ...
  ###Section 3.5: Updating existing sump depths
    ...
    #If there's new data to append
      ...
      tryCatch({outcome_accessdb_foundnewtables <- dbWriteTable(marsDBCon, 
  	  ...
  	  #Use this temp table to update the existing accessdb table
  	  dbGetQuery(marsDBCon, "update ow_sumpdepth s set sumpdepth_ft = t.sumpdepth_ft from ow_sumpdepth_temp t where s.ow_uid = t.ow_uid")
  	}
