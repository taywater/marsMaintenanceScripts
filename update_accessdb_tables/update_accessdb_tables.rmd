...
###Section 0.2: Connect to the database
	...
	#Also refresh the fieldwork materialized view
...
This script is attempting to update the `accessdb` and `ow_sumpdepth`tables in the `r params$database` database. It crawls the GSI Monitoring Sites folder, and does the following:  
  ...
  * For newly created Access DBs, parse their filepath and look in `ow_validity ow` for matching `ow.smp_id` and `ow.ow_suffix`
...
## State of site databases relative to `accessdb` and `ow_validity`

The site folders have been crawled and compared to the `accessdb` table and `ow_validity` materialized view.  
  
### New Access DBs to add to `accessdb`
  
There are `r nrow(appendtoaccessdbtable)` new Access DBs to add to the `accessdb` table. These DBs have defined OWs in the `ow_validity` materialized view.
