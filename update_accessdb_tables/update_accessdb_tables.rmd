...
This script is attempting to update the `accessdb` and `ow_sumpdepth`tables in the `r params$database` database. It crawls the GSI Monitoring Sites folder, and does the following:  
  ...
  * For newly created Access DBs, parse their filepath and look in `fieldwork.ow_all ow` for matching `ow.smp_id` and `ow.ow_suffix`
...
## State of site databases relative to `accessdb` and `fieldwork.ow_all`

The site folders have been crawled and compared to the `accessdb` table and `fieldwork.ow_all` table.  
  
### New Access DBs to add to `accessdb`
  
There are `r nrow(appendtoaccessdbtable)` new Access DBs to add to the `accessdb` table. These DBs have defined OWs in the `fieldwork.ow_all` table.
...
### Access DBs without a defined Observation Well in `fieldwork.ow_all`  
The script found `r nrow(accessdbswithoutow)` Access DBs without corresponding observation wells defined in `fieldwork.ow_all`. `r if(nrow(accessdbswithoutow) > 0) "Please create observation wells for these Access DBs with the \"Add Observation Well\" form in the MARS Fieldwork Tracking Shiny App"`  
