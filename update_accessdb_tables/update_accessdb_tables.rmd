...
errorCodes <- data.frame(code = 0:9,
  message = c("Execution successful.",
              ...
              "Access DBs orphaned (without tbl_ow entry) or missing (not at tbl_accessdb.filepath)",
...
```{r Section 0B - Preamble, include = FALSE}
    ...
    tryCatch(
        ...
        expr = {
          ...
          #Temporary table means R will copy the string "NA" into the table when a value is NA.
  	      ...
  	      dbGetQuery(marsDBCon, "update admin.tbl_accessdb a set sumptable = t.sumptable from tbl_accessdb_temp t where a.accessdb_uid = t.accessdb_uid
  	                              and t.sumptable <> 'NA'")
...
```{r Break Point 8 - Sumptable Failure, echo = FALSE, include = TRUE, eval = kill}
...
```{R Section R3 - Writing new sump depths to the DB, echo = FALSE, include = TRUE, eval = nrow(accessdbs_newmeasurement) > 0}
...
```{r Break Point 9 - Write Failure, echo = FALSE, include = TRUE, eval = kill}
...
```{R Section R4 - Updating existing sump depths in the DB, echo = FALSE, include = TRUE, eval = nrow(accessdbs_depthchanged) > 0}
    ...
    tryCatch(
        ...
        expr = {
          dbWriteTable(marsDBCon, DBI::SQL("tbl_sumpdepth_temp"), accessdbs_depthchanged,row.names = FALSE, overwrite = FALSE)

          #Use this temp table to update the existing table
  	      dbGetQuery(marsDBCon, "update fieldwork.tbl_ow_sumpdepth_intermediate s set sumpdepth_ft = t.sumpdepth_ft from tbl_sumpdepth_temp t where s.ow_uid = t.ow_uid")
...
```{r Break Point 10 - Update Failure, echo = FALSE, include = TRUE, eval = kill}
