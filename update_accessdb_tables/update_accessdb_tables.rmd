...
###Section 0.4: Parser functions
	...
	if(params$datestring == FALSE){
	  datestring <- strftime(lubridate::now(), format = "%Y%m%dT%H%M")
	} else {
...
This script is attempting to update the `admin.tbl_accessdb` and `fieldwork.tbl_ow_sumpdepth_intermediate`tables in the `r params$database` database. It crawls the GSI Monitoring Sites folder, and does the following:  
 ...
 1. Update the `admin.tbl_accessdb` table's set of information for GSI site Access DBs (`accessdb.filepath`, `accessdb.ow_uid`)  
  ...
  * For newly created Access DBs, parse their filepath and look in `fieldwork.tbl_ow ow` for matching `ow.smp_id` and `ow.ow_suffix`
  * Look for deleted/moved DBs, whose file paths can no longer be found
  
 2. Update the `admin.tbl_accessdb` table's set of canonical GSI table names (`accessdb.datatable`, `accessdb.sumptable`)
...
```{r Section 1A - New databases that have been created, include = FALSE}
  ...
  ###Section 1.4 Reconcile the new Access DBs with the existing defined OWs
    #1.4.1 New Access DBs that have matching defined OWs need can be added to the `admin.tbl_accessdb` table
    appendtoaccessdbtable <- filter(allnewaccessdbs_ow, !is.na(ow_uid)) %>% mutate(filepath = filepath_server)
    
    #Log outputs
    write_csv(appendtoaccessdbtable, file = paste0("logs/", datestring, "_appendtoaccessdbtable.csv"))

```    

## State of site databases relative to `admin.tbl_accessdb` and `fieldwork.tbl_ow`

The site folders have been crawled and compared to the `admin.tbl_accessdb` table and `fieldwork.tbl_ow` table.  
  
### New Access DBs to add to `admin.tbl_accessdb`
  
There are `r nrow(appendtoaccessdbtable)` new Access DBs to add to the `admin.tbl_accessdb` table. These DBs have defined OWs in the `fieldworktbl_ow` table.
...
```{r Section 1C - Incomplete entries, include = FALSE}
    ...
    #Log outputs
    write_csv(accessdbswithoutow, file = paste0("logs/", datestring, "_accessdbswithoutow.csv"))
    ...
    #Log outputs
    write_csv(existingdbs_notfound, file = paste0("logs/", datestring, "_existingdbs_notfound.csv"))
...
### Access DBs in `admin.tbl_accessdb` not found in the filesystem
...
### Access DBs without a defined Observation Well in `fieldwork.tbl_ow`  
The script found `r nrow(accessdbswithoutow)` Access DBs without corresponding observation wells defined in `fieldwork.tbl_ow`. `r if(nrow(accessdbswithoutow) > 0) "Please create observation wells for these Access DBs with the \"Add Observation Well\" form in the MARS Fieldwork Tracking Shiny App"`  
...
```{r Section 2 - Checking the Access DBs for canonical table names, include = FALSE}
  ...
  accessdbs <- dbGetQuery(marsDBCon, "SELECT * FROM admin.tbl_accessdb")
  ow <- dbGetQuery(marsDBCon, "SELECT * FROM fieldwork.tbl_ow")
  ...
  accessdb_tableguesses <- left_join(accessdbs, ow, by = "ow_uid") %>%
    ...
    # pick Q as a dumby drive, may need to change based on usage of the comp. handling the virtual task manager
    cmd_sub <- "subst Q: \"\\\\pwdoows\\OOWS\\Watershed Sciences\\GSI Monitoring\\02 GSI Monitoring Sites\""
    cmd_output <- system(cmd_sub, intern = TRUE)
    
    
    ## Trying RODBC method, 1/13/2023
    ...
    # #changing RODBC back to odbc, setting a shortcut for the file path to //pwdoows/oows/Watershed Sciences/GSI Monitoring/02 GSI Monitoring Sites/"
    # short_path <- gsub(pattern = "//pwdoows/oows/Watershed Sciences/GSI Monitoring/02 GSI Monitoring Sites/",
    #                    replacement = "Q:/",
    #                    x = accessdb_tableguesses$filepath[i])
    # 
    # accessdbCon <- odbc::dbConnect(odbc::odbc(),driver = 'Microsoft Access Driver (*.mdb, *.accdb)', database = short_path)
    # 
    # accessdbCon <- RODBC::odbcConnectAccess2007(access.file = accessdb_tableguesses$filepath[i], socket = default_socket())
    # 
  ...
  #Access DBs where we we found tables where we thought there were none
    ...
    #Log outputs
    write_csv(accessdb_foundnewtables, file = paste0("logs/", datestring, "_accessdb_foundnewtables.csv"))
  ...
  #Access DBs where we didn't find tables we thought were there
    ...
    #Log outputs
    write_csv(accessdb_missedsomething, file = paste0("logs/", datestring, "_accessdb_missedsomething.csv"))
  ...
  #Access DBs with no canonical tables
    ...
    #Log outputs
    write_csv(accessdb_nocanonical, file = paste0("logs/", datestring, "_accessdb_nocanonical.csv"))
...
The site databases in `admin.tbl_accessdb` have been inspected against the canonical tables in `accessdb.datatable` and `accessdb.sumptable`. Any changes have been summarized below.  
...
```{r Section 2.5 - Writing new canonical tables to accessdb table, include = FALSE}
  ...
  #If there's new data to append
    ...
    tryCatch({outcome_accessdb_newtablestowrite <- dbWriteTable(marsDBCon, 
	  ...
	  dbGetQuery(marsDBCon, "update admin.tbl_accessdb a set datatable = t.datatable from accessdb_temp t where a.accessdb_uid = t.accessdb_uid")
	  dbGetQuery(marsDBCon, "update admin.tbl_accessdb a set sumptable = t.sumptable from accessdb_temp t where a.accessdb_uid = t.accessdb_uid and t.sumptable <> 'NA'") #Temporary table means R will copy the string "NA" into the table when a value is NA. We must explicitly exclude these or else they'll end up in the final table and fuck everything up.
  }
```
...
```{r Section 3 - Gather data for new/updated sump depths, include=FALSE}
  ...
  #Log outputs
  write_csv(accessdbs_depthchanged, file = paste0("logs/", datestring, "_accessdbs_depthchanged.csv"))
  ...
  #Log outputs
  write_csv(accessdbs_newmeasurement, file = paste0("logs/", datestring, "_accessdbs_newmeasurement.csv"))
...
Changed sump depths measurements were discovered in `r nrow(accessdbs_depthchanged)` Access DBs.  
...
```{r Section 3.5 - Writing new measured sump depths to the ow_sumpdepth_intermediate table, include= FALSE}
  ...
  ###Section 3.5: Updating existing sump depths
    ...
    #If there's new data to append
      ...
      tryCatch({outcome_accessdb_foundnewtables <- dbWriteTable(marsDBCon, 
  	  ...
  	  #Use this temp table to update the existing accessdb table
  	  dbGetQuery(marsDBCon, "update fieldwork.tbl_ow_sumpdepth_intermediate s set sumpdepth_ft = t.sumpdepth_ft from ow_sumpdepth_temp t where s.ow_uid = t.ow_uid")
  	}
...
Updated sump depths were discovered in `r nrow(accessdbs_newmeasurement)` Access DBs.  
...
```{r Section 3.6 - Writing updates sump depths to the ow_sumpdepth table, include = FALSE}
  ###Section 3.6: Adding newly-created sump depths
    ...
    #If there's new data to append
  	if(nrow(accessdbs_newsumpdepths) > 0 & params$write == TRUE){
      tryCatch({outcome_accessdb_foundnewtables <- dbWriteTable(marsDBCon, 
          DBI::SQL("fieldwork.tbl_ow_sumpdepth_intermediate"), 
          accessdbs_newsumpdepths, 
          append = TRUE,
...
```{r Section 4 - Close DB connections and render this file, include = FALSE}
  ...
  # #Remove virtual drive shortcut
  # cmd_remove <- "subst M: /d"
  # cmd_output_remove <- system(cmd_remove, intern = TRUE)
