...
r_script = "'" + re.sub('\\\\', '\\\\\\\\',MAINTENANCEFOLDER) + "_pg9\\\\update_rainfall_tables\\\\update_rainfall_tables.rmd" + "'"
...
outputexists = os.path.isfile(MAINTENANCEFOLDER + "_pg9\\update_rainfall_tables\\" + re.sub('\\\\\\\\', '\\\\', output_file.strip("'")))
if outputexists:
	webbrowser.open(os.path.realpath(MAINTENANCEFOLDER + "_pg9\\update_rainfall_tables\\" + re.sub('\\\\\\\\', '\\\\', output_file.strip("'"))))
else:
	webbrowser.open(os.path.realpath(MAINTENANCEFOLDER + "_pg9\\update_rainfall_tables\\rainfall_error.html"))
