...
```{r Section 1 - Gathering rainfall data, include = FALSE}
  ...
  rainfall_gage <- dbGetQuery(marsDBCon, "SELECT * from data.gage_rain_latestdates")  
  
  #Read the rain gage table from the mars database and attach it to the rainfall data
  gage <- dbGetQuery(marsDBCon, "SELECT * FROM admin.gage")
...
## Updates to `data.gage_rain` Table

A new batch of rainfall data has been examined. There are `r nrow(newdata)` new rainfall measurements to add to the `data.gage_rain` table.
...
```{r Section 2 - Processing new rain events, include = FALSE}
  ...
  newrain_query <- "select rg.gage_uid, rg.dtime_edt, rg.rainfall_in from data.gage_rain rg left join data.gage_event_latestdates rgel on rg.gage_uid = rgel.gage_uid where rg.dtime_edt > rgel.dtime_edt"
  ...
  emptytable <- dbGetQuery(marsDBCon, "select count(*) from data.gage_event_latestdates") %>% pull(count)
  if(emptytable == 0){
    rain_newdata <- dbGetQuery(marsDBCon, "select dtime_edt, gage_uid, rainfall_in from data.gage_rain")
  }
...
```{r Section 3 - Gathering radar data, include = FALSE}
  ...
  db_rawfiles <- dbGetQuery(marsDBCon, "select * from admin.radar_rawfile")
  ...
  phillycells <- dbGetQuery(marsDBCon, "select radar_uid from admin.radar")
...
## Updates to the `admin.radar_rawfile` and `data.radar_rain` Table

A new batch of rainfall data has been examined. There are `r nrow(newradardata)` new rainfall measurements to add to the `data.radar_rain` table.

`r if(nrow(newradardata) > 0){ group_by(newradardata, radar_uid) %>% arrange(dtime_edt) %>% summarize(new_measurements = n(), first_date = first(dtime_edt), last_date = last(dtime_edt)) %>% kable }`

```{r Section 3.5 - Writing to the radar tables, include = FALSE}  
# Fail safe. If the block below doesn't run, we didn't append anything
...
`r if(nrow(new_rawfiles) > 0){ paste("New radar rawfiles have", ifelse(outcome_rawfiles, "SUCCESSFULLY", "NOT"), "been written to the admin.radar_rawfile table")}`

`r if(nrow(newradardata) > 0){ paste("New radar data has", ifelse(outcome_radardata, "SUCCESSFULLY", "NOT"), "been written to the data.radar_rain table")}`
...
```{r Section 4 - Processing new rain events for radar data, include = FALSE}
  ...
  allcells <- dbGetQuery(marsDBCon, "select distinct radar_uid from admin.radar")

  newrain_query <- paste("select rc.radar_uid, rc.dtime_edt, rc.rainfall_in from data.radar_rain rc left join data.radar_event_latestdates rcel on rc.radar_uid = rcel.radar_uid where rc.dtime_edt > rcel.dtime_edt")
...
## Updates to `data.radar_event` Table

The latest rainfall data has been processed for event detection. There are `r nrow(rain_newevents)` new rain events to add to the `data.radar_event` table.

`r if(nrow(rain_newevents) > 0){ rain_newevents %>% head(100) %>% kable }`


```{r Section 4.5 - Writing to the data.radar_event table, include = FALSE}  
# Fail safe. If the block below doesn't run, we didn't append anything
  outcome_rain_newevents <- FALSE
  #If there's new data to append
  if(nrow(rain_newevents) > 0 & params$write == TRUE){
    outcome_rain_totalevents <- tryCatch({outcome_rain_newevents <- dbWriteTable(marsDBCon, 
        DBI::SQL("data.radar_event"), 
        rain_newevents, 
        append= TRUE, 
        row.names = FALSE)
      }, # append the data
...
`r if(nrow(newdata) > 0){ paste("New rain events have", ifelse(outcome_rain_totalevents, "SUCCESSFULLY", "NOT"), "been written to the data.radar_event table")}`
