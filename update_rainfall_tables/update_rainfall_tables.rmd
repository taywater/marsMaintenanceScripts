...
```{r Section 1 - Gathering rainfall data, include = FALSE}
  ...
  newdata$dtime_edt %<>% as.character #prevents drift due to time zone defaults
  
  #In certain rare circumstances, duplicate data can be missed. I'm not sure why this happens, but we can filter it out with relative ease.
  keys <- dbGetQuery(marsDBCon, "select concat(gage_uid, '_', dtime_edt) as key from rainfall_gage_raw") %>% pull(key)
  newdata %<>% mutate(key = paste(gage_uid, dtime_edt, sep = "_"))
 newdata <- filter(newdata, !(key %in% keys)) %>% select(-key)
  
```

## Updates to `rainfall_gage_raw` Table

A new batch of rainfall data has been examined. There are `r nrow(newdata)` new rainfall measurements to add to the `rainfall_gage_raw` table.
...
```{r Section 2 - Processing new rain events, include = FALSE}
  ...
  rain_newevents <- rain_newdata %>% 
    ...
    summarize(eventdatastart_edt = as.character(first(dtime_edt)), #prevents time zone drift
      eventdataend_edt = as.character(last(dtime_edt)), #prevents time zone drift
  ...
  rain_newevents$eventdatastart_edt %<>% as.character #prevent drift
  rain_newevents$eventdataend_edt %<>% as.character #prevent drift
```
...
```{r Section 3 - Gathering radar data, include = FALSE}
  ...
  new_rawfiles <- filter(hh_rawfiles, !(filepath %in% db_rawfiles$filepath), yearmon > "201301")
  ...
  for(i in 1:nrow(new_rawfiles)){
    ...
    unzip(paste0("~/radarrain/", basename(new_rawfiles$filepath[i])), exdir = "~/radarrain")
  ...
  newradardata$dtime_edt %<>% as.character #Prevent time zone drift
  
```

## Updates to `rainfall_radarcell_raw` Table

A new batch of rainfall data has been examined. There are `r nrow(newradardata)` new rainfall measurements to add to the `rainfall_radarcell_raw` table.

`r if(nrow(newradardata) > 0){ group_by(newradardata, radarcell_uid) %>% arrange(dtime_edt) %>% summarize(new_measurements = n(), first_date = first(dtime_edt), last_date = last(dtime_edt)) %>% kable }`
...
`r if(nrow(newradardata) > 0){ paste("New rainfall measurements have", ifelse(outcome_radardata, "SUCCESSFULLY", "NOT"), "been written to the rainfall_radarcell_raw table")}`
...
## Updates to `rainfall_radarcell_event` Table

The latest rainfall data has been processed for event detection. There are `r nrow(rain_newevents)` new rain events to add to the `rainfall_radarcell_event` table.
...
`r if(nrow(rain_newevents) > 0){ paste("New rain events have", ifelse(outcome_rain_newevents, "SUCCESSFULLY", "NOT"), "been written to the rainfall_radarcell_event table")}`

```{r Section 5 - Close DB connections and render this file, include = FALSE}
