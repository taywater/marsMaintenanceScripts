...
```{r Section 3 - Gathering radar data, include = FALSE}
  ...
  for(i in 1:length(newradarfiles)){
    ...
    radarfile <- radarfile %>% 
      filter(rainfall_in != 0, radarcell_uid %in% phillycells$radarcell_uid) %>%
...
```{r Section 4 - Processing new rain events for radarcell data, include = FALSE}
  ...
  for(i in 1:length(allcells$radarcell_uid)){
    newrain_query <- paste("select rc.radarcell_uid, rc.dtime_edt, rc.rainfall_in from rainfall_radarcell_raw rc left join rainfall_radarcell_event_latestdates rcel on rc.radarcell_uid = rcel.radarcell_uid where rc.dtime_edt > rcel.dtime_edt and rc.radarcell_uid =", allcells$radarcell_uid[i])

    
    print(paste("Rain events for cell", allcells$radarcell_uid[i]))
    ...
    rain_newevents <- rain_newdata %>% 
      ...
      if(i == 1){
        rain_totalevents <- rain_newevents
      }
      if(i != 1){
        rain_totalevents <- bind_rows(rain_totalevents, rain_newevents)
      }  
...
# Fail safe. If the block below doesn't run, we didn't append anything
  ...
  #If there's new data to append
  if(nrow(rain_totalevents) > 0 & params$write == TRUE){
