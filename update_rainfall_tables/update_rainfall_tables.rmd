...
## Updates to `data.tbl_gage_rain` Table

A new batch of rainfall data has been examined. There are `r nrow(newdata)` new rainfall measurements to add to the `data.tbl_gage_rain` table.
...
## Updates to `tbl_gage_event` Table

The latest rainfall data has been processed for event detection. There are `r nrow(rain_newevents)` new rain events to add to the `tbl_gage_event` table.
...
## Updates to the `admin.tbl_radar_rawfile` and `data.tbl_radar_rain` Table

A new batch of rainfall data has been examined. There are `r nrow(newradardata)` new rainfall measurements to add to the `data.tbl_radar_rain` table.
...
# Fail safe. If the block below doesn't run, we didn't append anything
  ...
  if(nrow(new_rawfiles) > 0 & params$write == TRUE){
    ...
    outcome_rawfiles <- tryCatch({outcome_rawfiles <- dbWriteTable(marsDBCon, 
        DBI::SQL("admin.tbl_radar_rawfile"), 
        new_rawfiles, 
        append= TRUE, 
        row.names = FALSE)
      }, # append the data
...
`r if(nrow(new_rawfiles) > 0){ paste("New radar rawfiles have", ifelse(outcome_rawfiles, "SUCCESSFULLY", "NOT"), "been written to the admin.tbl_radar_rawfile table")}`

`r if(nrow(newradardata) > 0){ paste("New radar data has", ifelse(outcome_radardata, "SUCCESSFULLY", "NOT"), "been written to the data.tbl_radar_rain table")}`
...
## Updates to `data.tbl_radar_event` Table

The latest rainfall data has been processed for event detection. There are `r nrow(rain_newevents)` new rain events to add to the `data.tbl_radar_event` table.

`r if(nrow(rain_newevents) > 0){ rain_newevents %>% head(100) %>% kable }`


```{r Section 4.5 - Writing to the data.tbl_radar_event table, include = FALSE}  
# Fail safe. If the block below doesn't run, we didn't append anything
  outcome_rain_totalevents <- FALSE
  #If there's new data to append
  if(nrow(rain_newevents) > 0 & params$write == TRUE){
    outcome_rain_totalevents <- tryCatch({outcome_rain_totalevents <- dbWriteTable(marsDBCon, 
        DBI::SQL("data.tbl_radar_event"), 
        rain_newevents, 
        append= TRUE, 
        row.names = FALSE)
      }, # append the data
...
`r if(nrow(newdata) > 0){ paste("New rain events have", ifelse(outcome_rain_totalevents, "SUCCESSFULLY", "NOT"), "been written to the data.tbl_radar_event table")}`
