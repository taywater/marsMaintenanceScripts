...
r_script = "'" + re.sub('\\\\', '\\\\\\\\',MAINTENANCEFOLDER) + "_pg9\\\\update_smp_tables\\\\update_smp_tables.rmd'"
...
outputexists = os.path.isfile(MAINTENANCEFOLDER + "_pg9\\update_smp_tables\\" + re.sub('\\\\\\\\', '\\\\', output_file.strip("'")))
if outputexists:
  webbrowser.open(os.path.realpath(MAINTENANCEFOLDER + "_pg9\\update_smp_tables\\" + re.sub('\\\\\\\\', '\\\\', output_file.strip("'"))))
else:
  webbrowser.open(os.path.realpath(MAINTENANCEFOLDER + "_pg9\\update_smp_tables\\smp_error.html"))