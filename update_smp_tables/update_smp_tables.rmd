...
###Check GIS data for any new SMPs, and insert them into smp_loc
	...
	smp_loc <- dbGetQuery(marsDBCon, "SELECT * FROM admin.smp_loc")
...
## Updates to `admin.smp_loc` Table

New centroids have been exported and examined against the existing `admin.smp_loc` table. There are `r nrow(smp_locNewData)` new centroids to add to the `admin.smp_loc` table.
...
```{r Section 1.5 - Writing to the smp_loc table, include = FALSE}	
  ...
  #If there's new data to append
    ...
    tryCatch({outcome_centroids <- dbWriteTable(marsDBCon, DBI::SQL("admin.smp_loc"), smp_locNewData, append= TRUE, row.names = FALSE)}, # append the data
             error = function(e) e
    )
  }
```
...
###If there are any SMPs with locations that do not have rain gage associations, apply those now
	...
	smp_gage <- dbGetQuery(marsDBCon, "SELECT * FROM admin.smp_gage")
	...
	if(nrow(smp_loc_nogage) > 0){
		...
		gage_loc <- dbGetQuery(marsDBCon, "SELECT * FROM admin.gage_loc")
...
## Updates to `admin.smp_gage` Table

The `admin.smp_gage` table has been checked for SMPs without a rain gage association. There are `r nrow(smp_loc_nogage)` new entries to add to the `admin.smp_gage` table.
...
```{r Section 2.5 - Writing to the smp_gage table, include = FALSE}
    ...
    if(nrow(smp_loc_nogage) > 0 & params$write == TRUE){
		  tryCatch({outcome_gage <- dbWriteTable(marsDBCon, DBI::SQL("admin.smp_gage"), smp_loc_nogage, append= TRUE, row.names = FALSE)}, # append the data	
		           error = function(e) e
		  )
	  }
```

New gage associations have `r ifelse(outcome_gage, "SUCCESSFULLY", "NOT")` been appended to the `admin.smp_gage` table.  
...
###If there are any SMPs with locations that do not have radar associations, apply those now
	...
	smp_radar <- dbGetQuery(marsDBCon, "SELECT * FROM admin.smp_radar")
	...
	if(nrow(smp_loc_noradar) > 0){
		...
		radar_loc <- dbGetQuery(marsDBCon, "SELECT * FROM admin.radar_loc")
...
## Updates to `admin.smp_radar` Table

The `admin.smp_radar` table has been checked for SMPs without a radar association. There are `r nrow(smp_loc_noradar)` new entries to add to the `admin.smp_radar` table.

`r if(nrow(smp_loc_noradar) > 0){knitr::kable(smp_loc_noradar)}`

```{r Section 3.5 - Writing to the smp_radar table, include = FALSE}
    #Fail safe. If the block below doesn't run, we didn't append anything
	outcome_radar <- FALSE

	#If there's new data to append
    if(nrow(smp_loc_noradar) > 0 & params$write == TRUE){
		  tryCatch({outcome_radar <- dbWriteTable(marsDBCon, DBI::SQL("admin.smp_radar"), smp_loc_noradar, append= TRUE, row.names = FALSE)}, # append the data	
		           error = function(e) e
		  )
	  }
```

New radar associations have `r ifelse(outcome_radar, "SUCCESSFULLY", "NOT")` been appended to the `admin.smp_radar` table.  
