...
order by p.spra_trackingid, s.spra_name")
   ...
   tryCatch({head_vals <- dbGetQuery(planreview, "select       spra_name as 'smp_id',
                                                  ...
                                                  coalesce(spra_staticstorage_asmaintained, spra_staticstorage_asbuilt, spra_staticstorage_approved) as                                                                        'static_storage_vol_ft3',
                                                  coalesce(spra_waterqualityvolume_asmaintained, spra_waterqualityvolume_asbuilt, spra_waterqualityvolume_approved) as                                                        'water_quality_vol_ft3'
...
#Write to mars_data
  ...
  #append new SMPs
      tryCatch(
        ...
        expr = {
          dbWriteTable(mars_data, DBI::SQL("external.tbl_planreview_private_fixed"), new_smp_hashes, append= TRUE, row.names = FALSE)
          success <<- TRUE
          },
        error = function(e) {
          kill <<- TRUE
      ...
      tryCatch(
        ...
        expr = {
          dbWriteTable(mars_data, DBI::SQL("external.tbl_planreview_crosstab"), new_crosstab_hashes, append= TRUE, row.names = FALSE)
          success <<- TRUE
          },
        error = function(e) {
          kill <<- TRUE
  ...
  if(!kill)
  {
    ...
    dbWriteTable(mars_data, DBI::SQL("log.tbl_writes_planreview"), logMessage, append = TRUE, row.names=FALSE) 
  }
