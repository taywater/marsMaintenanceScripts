...
mars <- dbConnect(odbc(), "mars_data14")
...
```{r Section 1 - Gathering data from pwdgis4, echo = FALSE, eval = keepRunning}
...
```{r Section 2 - Hashing Tables, echo = FALSE, eval = keepRunning}  
# hash the tables
    hash_basin <- basin %>%
...
```{r Section 3 - Pull existing tables from MARS PG14 database, include=FALSE, eval = keepRunning}
...
```{r Section 4 - Identify differnces between tables, include=FALSE, eval = keepRunning}
...
```{r Section We Are As One - external.tbl_gswibasin, echo = FALSE, eval = all(keepRunning, newAssets, nrow(new_basin) > 0)}
###New SMP Polygons
knitr::asis_output(paste("### New assets to add to `external.tbl_gswibasin`: ",   nrow(new_basin)))
kable(head(new_basin))
```

```{r Section We Are As One - external.tbl_gswiblueroof, echo = FALSE, eval = all(keepRunning, newAssets, nrow(new_blueroof) > 0)}
knitr::asis_output(paste("### New assets to add to `external.tbl_gswiblueroof`: ",   nrow(new_blueroof)))
kable(head(new_blueroof))
```

```{r Section We Are As One - external.tbl_gswibumpout, echo = FALSE, eval = all(keepRunning, newAssets, nrow(new_bumpout) > 0)}
knitr::asis_output(paste("### New assets to add to `external.tbl_gswibumpout`: ",   nrow(new_bumpout)))
kable(head(new_bumpout))
```

```{r Section We Are As One - external.tbl_gswicistern, echo = FALSE, eval = all(keepRunning, newAssets, nrow(new_cistern) > 0)}
knitr::asis_output(paste("### New assets to add to `external.tbl_gswicistern`: ",   nrow(new_cistern)))
kable(head(new_cistern))
```

```{r Section We Are As One - external.tbl_gswidrainagewell, echo = FALSE, eval = all(keepRunning, newAssets, nrow(new_drainagewell) > 0)}
...
```{r Section We Are As One - external.tbl_gswigreenroof, echo = FALSE, eval = all(keepRunning, newAssets, nrow(new_greenroof) > 0)}
knitr::asis_output(paste("### New assets to add to `external.tbl_gswigreenroof`: ",   nrow(new_greenroof)))
kable(head(new_greenroof))
```

```{r Section We Are As One - external.tbl_gswipermeablepavement, echo = FALSE, eval = all(keepRunning, newAssets, nrow(new_permeablepavement) > 0)}
...
```{r Section We Are As One - external.tbl_gswiplanter, echo = FALSE, eval = all(keepRunning, newAssets, nrow(new_planter) > 0)}
knitr::asis_output(paste("### New assets to add to `external.tbl_gswiplanter`: ",   nrow(new_planter)))
kable(head(new_planter))
```

```{r Section We Are As One - external.tbl_gswiraingarden, echo = FALSE, eval = all(keepRunning, newAssets, nrow(new_raingarden) > 0)}
knitr::asis_output(paste("### New assets to add to `external.tbl_gswiraingarden`: ",   nrow(new_raingarden)))
kable(head(new_raingarden))
```

```{r Section We Are As One - external.tbl_gswiswale, echo = FALSE, eval = all(keepRunning, newAssets, nrow(new_swale) > 0)}
knitr::asis_output(paste("### New assets to add to `external.tbl_gswiswale`: ",   nrow(new_swale)))
kable(head(new_swale))
```

```{r Section We Are As One - external.tbl_gswitree, echo = FALSE, eval = all(keepRunning, newAssets, nrow(new_tree) > 0)}
knitr::asis_output(paste("### New assets to add to `external.tbl_gswitree`: ",   nrow(new_tree)))
kable(head(new_tree))
```

```{r Section We Are As One - external.tbl_gswitreetrench, echo = FALSE, eval = all(keepRunning, newAssets, nrow(new_treetrench) > 0)}
knitr::asis_output(paste("### New assets to add to `external.tbl_gswitreetrench`: ",   nrow(new_treetrench)))
kable(head(new_treetrench))
```

```{r Section We Are As One - external.tbl_gswitrench, echo = FALSE, eval = all(keepRunning, newAssets, nrow(new_trench) > 0)}
knitr::asis_output(paste("### New assets to add to `external.tbl_gswitrench`: ",   nrow(new_trench)))
kable(head(new_trench))
```

```{r Section We Are As One - external.tbl_gswiwetland, echo = FALSE, eval = all(keepRunning, newAssets, nrow(new_wetland) > 0)}
knitr::asis_output(paste("### New assets to add to `external.tbl_gswiwetland`: ",   nrow(new_wetland)))
kable(head(new_wetland))
```

```{r Section We Are As One - external.tbl_gswicleanout, echo = FALSE, eval = all(keepRunning, newAssets, nrow(new_cleanout) > 0)}
###New SMP Components
knitr::asis_output(paste("### New assets to add to `external.tbl_gswicleanout`: ",   nrow(new_cleanout)))
kable(head(new_cleanout))
```

```{r Section We Are As One - external.tbl_gswicontrolstructure, echo = FALSE, eval = all(keepRunning, newAssets, nrow(new_controlstructure) > 0)}
...
```{r Section We Are As One - external.tbl_gswiconveyance, echo = FALSE, eval = all(keepRunning, newAssets, nrow(new_conveyance) > 0)}
knitr::asis_output(paste("### New assets to add to `external.tbl_gswiconveyance`: ",   nrow(new_conveyance)))
kable(head(new_conveyance))
```

```{r Section We Are As One - external.tbl_gswifitting, echo = FALSE, eval = all(keepRunning, newAssets, nrow(new_fitting) > 0)}
knitr::asis_output(paste("### New assets to add to `external.tbl_gswifitting`: ",   nrow(new_fitting)))
kable(head(new_fitting))
```

```{r Section We Are As One - external.tbl_gswiinlet, echo = FALSE, eval = all(keepRunning, newAssets, nrow(new_inlet) > 0)}
knitr::asis_output(paste("### New assets to add to `external.tbl_gswiinlet`: ",   nrow(new_inlet)))
kable(head(new_inlet))
```

```{r Section We Are As One - external.tbl_gswimanhole, echo = FALSE, eval = all(keepRunning, newAssets, nrow(new_manhole) > 0)}
knitr::asis_output(paste("### New assets to add to `external.tbl_gswimanhole`: ",   nrow(new_manhole)))
kable(head(new_manhole))
```

```{r Section We Are As One - external.tbl_gswiobservationwell, echo = FALSE, eval = all(keepRunning, newAssets, nrow(new_observationwell) > 0)}
...
```{r Section We Are As One - external.tbl_gswistructure, echo = FALSE, eval = all(keepRunning, newAssets, nrow(new_structure) > 0)}
knitr::asis_output(paste("### New assets to add to `external.tbl_gswistructure`: ",   nrow(new_structure)))
kable(head(new_structure))
```

```{r Section We Are As One - Write Everything, echo = FALSE, eval = all(keepRunning, newAssets)}
#Write new assets
if(newAssets){
...
```{r Section X abc, echo = FALSE, include = FALSE, eval = keepRunning}
...
```{r Section We Are As One - Updates to external.tbl_gswibasin, echo = FALSE, eval = all(keepRunning, newUpdates, nrow(update_basin) > 0)}
knitr::asis_output(paste("### Assets to update in `external.tbl_gswibasin`: ",   nrow(update_basin)))
kable(head(update_basin))
```

```{r Section We Are As One - Updates to external.tbl_gswiblueroof, echo = FALSE, eval = all(keepRunning, newUpdates, nrow(update_blueroof) > 0)}
knitr::asis_output(paste("### Assets to update in `external.tbl_gswiblueroof`: ",   nrow(update_blueroof)))
kable(head(update_blueroof))
```

```{r Section We Are As One - Updates to external.tbl_gswibumpout, echo = FALSE, eval = all(keepRunning, newUpdates, nrow(update_bumpout) > 0)}
knitr::asis_output(paste("### Assets to update in `external.tbl_gswibumpout`: ",   nrow(update_bumpout)))
kable(head(update_bumpout))
```

```{r Section We Are As One - Updates to external.tbl_gswicistern, echo = FALSE, eval = all(keepRunning, newUpdates, nrow(update_cistern) > 0)}
knitr::asis_output(paste("### Assets to update in `external.tbl_gswicistern`: ",   nrow(update_cistern)))
kable(head(update_cistern))
```

```{r Section We Are As One - Updates to external.tbl_gswidrainagewell, echo = FALSE, eval = all(keepRunning, newUpdates, nrow(update_drainagewell) > 0)}
...
```{r Section We Are As One - Updates to external.tbl_gswigreenroof, echo = FALSE, eval = all(keepRunning, newUpdates, nrow(update_greenroof) > 0)}
knitr::asis_output(paste("### Assets to update in `external.tbl_gswigreenroof`: ",   nrow(update_greenroof)))
kable(head(update_greenroof))
```

```{r Section We Are As One - Updates to external.tbl_gswipermeablepavement, echo = FALSE, eval = all(keepRunning, newUpdates, nrow(update_permeablepavement) > 0)}
...
```{r Section We Are As One - Updates to external.tbl_gswiplanter, echo = FALSE, eval = all(keepRunning, newUpdates, nrow(update_planter) > 0)}
knitr::asis_output(paste("### Assets to update in `external.tbl_gswiplanter`: ",   nrow(update_planter)))
kable(head(update_planter))
```

```{r Section We Are As One - Updates to external.tbl_gswiraingarden, echo = FALSE, eval = all(keepRunning, newUpdates, nrow(update_raingarden) > 0)}
...
```{r Section We Are As One - Updates to external.tbl_gswiswale, echo = FALSE, eval = all(keepRunning, newUpdates, nrow(update_swale) > 0)}
knitr::asis_output(paste("### Assets to update in `external.tbl_gswiswale`: ",   nrow(update_swale)))
kable(head(update_swale))
```

```{r Section We Are As One - Updates to external.tbl_gswitree, echo = FALSE, eval = all(keepRunning, newUpdates, nrow(update_tree) > 0)}
knitr::asis_output(paste("### Assets to update in `external.tbl_gswitree`: ",   nrow(update_tree)))
kable(head(update_tree))
```

```{r Section We Are As One - Updates to external.tbl_gswitreetrench, echo = FALSE, eval = all(keepRunning, newUpdates, nrow(update_treetrench) > 0)}
...
```{r Section We Are As One - Updates to external.tbl_gswitrench, echo = FALSE, eval = all(keepRunning, newUpdates, nrow(update_trench) > 0)}
knitr::asis_output(paste("### Assets to update in `external.tbl_gswitrench`: ",   nrow(update_trench)))
kable(head(update_trench))
```

```{r Section We Are As One - Updates to external.tbl_gswiwetland, echo = FALSE, eval = all(keepRunning, newUpdates, nrow(update_wetland) > 0)}
knitr::asis_output(paste("### Assets to update in `external.tbl_gswiwetland`: ",   nrow(update_wetland)))
kable(head(update_wetland))
```

```{r Section We Are As One - Updates to external.tbl_gswicleanout, echo = FALSE, eval = all(keepRunning, newUpdates, nrow(update_cleanout) > 0)}
knitr::asis_output(paste("### Assets to update in `external.tbl_gswicleanout`: ",   nrow(update_cleanout)))
kable(head(update_cleanout))
```

```{r Section We Are As One - Updates to external.tbl_gswicontrolstructure, echo = FALSE, eval = all(keepRunning, newUpdates, nrow(update_controlstructure) > 0)}
...
```{r Section We Are As One - Updates to external.tbl_gswiconveyance, echo = FALSE, eval = all(keepRunning, newUpdates, nrow(update_conveyance) > 0)}
...
```{r Section We Are As One - Updates to external.tbl_gswifitting, echo = FALSE, eval = all(keepRunning, newUpdates, nrow(update_fitting) > 0)}
knitr::asis_output(paste("### Assets to update in `external.tbl_gswifitting`: ",   nrow(update_fitting)))
kable(head(update_fitting))
```

```{r Section We Are As One - Updates to external.tbl_gswiinlet, echo = FALSE, eval = all(keepRunning, newUpdates, nrow(update_inlet) > 0)}
knitr::asis_output(paste("### Assets to update in `external.tbl_gswiinlet`: ",   nrow(update_inlet)))
kable(head(update_inlet))
```

```{r Section We Are As One - Updates to external.tbl_gswimanhole, echo = FALSE, eval = all(keepRunning, newUpdates, nrow(update_manhole) > 0)}
knitr::asis_output(paste("### Assets to update in `external.tbl_gswimanhole`: ",   nrow(update_manhole)))
kable(head(update_manhole))
```

```{r Section We Are As One - Updates to external.tbl_gswiobservationwell, echo = FALSE, eval = all(keepRunning, newUpdates, nrow(update_observationwell) > 0)}
...
```{r Section We Are As One - Updates to external.tbl_gswistructure, echo = FALSE, eval = all(keepRunning, newUpdates, nrow(update_structure) > 0)}
knitr::asis_output(paste("### Assets to update in `external.tbl_gswistructure`: ",   nrow(update_structure)))
kable(head(update_structure))
```

```{r Section We Are As One - Update Everything, echo = FALSE, eval = all(keepRunning, newUpdates)}
#Write new assets
if(newUpdates){
...
# Script Results: `r ifelse(keepRunning, "SUCCESS", "FAILURE")`
