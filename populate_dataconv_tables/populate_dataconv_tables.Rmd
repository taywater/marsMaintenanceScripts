...
#Query the DB to pull the existing versions
    ...
    bind_rows(db_basin, db_blueroof, db_bumpout, db_cistern, db_drainagewell, db_greenroof, db_permeablepavement, db_planter, db_raingarden, db_swale, db_tree, db_treetrench, db_trench, db_wetland)
...
```{r Section 5 - Write Assets, echo = FALSE, eval = all(keepRunning, newAssets))}
###New SMP Polygons
...
```{r Section X abc, echo = FALSE, eval = keepRunning}
...
#Update old assets
    ...
    if(nrow(update_structure) > 0){
        ...
        dbBind(update_gswistructure, new_structure)
...
#Sum new SMPs
new_smp_count <-  nrow(new_basin) + nrow(new_blueroof) + nrow(new_bumpout) + nrow(new_cistern) + nrow(new_drainagewell) + nrow(new_greenroof) + nrow(new_permeablepavement) + nrow(new_planter) + nrow(new_raingarden) + nrow(new_swale) + nrow(new_tree) + nrow(new_treetrench) + nrow(new_trench) + nrow(new_wetland) 

#Sum new structures
new_structure_count <- nrow(new_cleanout) + nrow(new_controlstructure) + nrow(new_conveyance) + nrow(new_fitting) + nrow(new_inlet) + nrow(new_manhole) + nrow(new_observationwell) + nrow(new_structure)
    
#Sum Updated SMPs
updated_smp_count <-  nrow(update_basin) + nrow(update_blueroof) + nrow(update_bumpout) + nrow(update_cistern) + nrow(update_drainagewell) + nrow(update_greenroof) + nrow(update_permeablepavement) + nrow(update_planter) + nrow(update_raingarden) + nrow(update_swale) + nrow(update_tree) + nrow(update_treetrench) + nrow(update_trench) + nrow(update_wetland)

#Sum Updated structures
updated_structure_count <- nrow(update_cleanout) + nrow(update_controlstructure) + nrow(update_conveyance) + nrow(update_fitting) + nrow(update_inlet) + nrow(update_manhole) + nrow(update_observationwell) + nrow(update_structure)
...
`r if(nrow(new_basin)>0){knitr::kable(new_basin, caption = "Basins")}`
`r if(nrow(new_blueroof)>0){knitr::kable(new_blueroof, caption = "Blue Roofs")}`
`r if(nrow(new_bumpout)>0){knitr::kable(new_bumpout, caption = "Bumpouts")}`
`r if(nrow(new_cistern)>0){knitr::kable(new_cistern, caption = "Cisterns")}`
`r if(nrow(new_drainagewell)>0){knitr::kable(new_drainagewell, caption = "Drainage Wells")}`
`r if(nrow(new_greenroof)>0){knitr::kable(new_greenroof, caption = "Green Roofs")}`
`r if(nrow(new_permeablepavement)>0){knitr::kable(new_permeablepavement, caption = "Permeable Pavement")}`
`r if(nrow(new_planter)>0){knitr::kable(new_planter, caption = "Planters")}`
`r if(nrow(new_raingarden)>0){knitr::kable(new_raingarden, caption = "Rain Gardens")}`
`r if(nrow(new_swale)>0){knitr::kable(new_swale, caption = "Swales")}`
`r if(nrow(new_tree)>0){knitr::kable(new_tree, caption = "Trees")}`
`r if(nrow(new_treetrench)>0){knitr::kable(new_treetrench, caption = "Tree Trenches")}`
`r if(nrow(new_trench)>0){knitr::kable(new_trench, caption = "Trenches")}`
`r if(nrow(new_wetland)>0){knitr::kable(new_wetland, caption = "Wetlands")}`
...
`r if(nrow(new_cleanout)>0){knitr::kable(new_cleanout, caption = "Cleanouts")}`
`r if(nrow(new_controlstructure)>0){knitr::kable(new_controlstructure, caption = "Control Structures")}`
`r if(nrow(new_conveyance)>0){knitr::kable(new_conveyance, caption = "Conveyance")}`
`r if(nrow(new_fitting)>0){knitr::kable(new_fitting, caption = "Fittings")}`
`r if(nrow(new_inlet)>0){knitr::kable(new_inlet, caption = "Inlets")}`
`r if(nrow(new_manhole)>0){knitr::kable(new_manhole, caption = "Manholes")}`
`r if(nrow(new_observationwell)>0){knitr::kable(new_observationwell, caption = "Observation Wells")}`
`r if(nrow(new_structure)>0){knitr::kable(new_structure, caption = "Structures")}`
...
`r if(nrow(update_basin)>0){knitr::kable(update_basin, caption = "Basins")}`
`r if(nrow(update_blueroof)>0){knitr::kable(update_blueroof, caption = "Blue Roofs")}`
`r if(nrow(update_bumpout)>0){knitr::kable(update_bumpout, caption = "Bumpouts")}`
`r if(nrow(update_cistern)>0){knitr::kable(update_cistern, caption = "Cisterns")}`
`r if(nrow(update_drainagewell)>0){knitr::kable(update_drainagewell, caption = "Drainage Wells")}`
`r if(nrow(update_greenroof)>0){knitr::kable(update_greenroof, caption = "Green Roofs")}`
`r if(nrow(update_permeablepavement)>0){knitr::kable(update_permeablepavement, caption = "Permeable Pavement")}`
`r if(nrow(update_planter)>0){knitr::kable(update_planter, caption = "Planters")}`
`r if(nrow(update_raingarden)>0){knitr::kable(update_raingarden, caption = "Rain Gardens")}`
`r if(nrow(update_swale)>0){knitr::kable(update_swale, caption = "Swales")}`
`r if(nrow(update_tree)>0){knitr::kable(update_tree, caption = "Trees")}`
`r if(nrow(update_treetrench)>0){knitr::kable(update_treetrench, caption = "Tree Trenches")}`
`r if(nrow(update_trench)>0){knitr::kable(update_trench, caption = "Trenches")}`
`r if(nrow(update_wetland)>0){knitr::kable(update_wetland, caption = "Wetlands")}`
...
`r if(nrow(update_cleanout)>0){knitr::kable(update_cleanout, caption = "Cleanouts")}`
`r if(nrow(update_controlstructure)>0){knitr::kable(update_controlstructure, caption = "Control Structures")}`
`r if(nrow(update_conveyance)>0){knitr::kable(update_conveyance, caption = "Conveyance")}`
`r if(nrow(update_fitting)>0){knitr::kable(update_fitting, caption = "Fittings")}`
`r if(nrow(update_inlet)>0){knitr::kable(update_inlet, caption = "Inlets")}`
`r if(nrow(update_manhole)>0){knitr::kable(update_manhole, caption = "Manholes")}`
`r if(nrow(update_observationwell)>0){knitr::kable(update_observationwell, caption = "Observation Wells")}`
`r if(nrow(update_structure)>0){knitr::kable(update_structure, caption = "Structures")}`
