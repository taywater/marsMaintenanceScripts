...
r_script = "'" + re.sub('\\\\', '\\\\\\\\',MAINTENANCEFOLDER) + "\\\\populate_dataconv_tables\\\\populate_dataconv_tables.rmd'"
database = "'mars_data'"
writeflag = "FALSE"
output_file = "'output\\\\" + datestring + "populate_dataconv_tables.html" + "'"
...
outputexists = os.path.isfile(MAINTENANCEFOLDER + "\\populate_dataconv_tables\\" + re.sub('\\\\\\\\', '\\\\', output_file.strip("'")))
if outputexists:
	webbrowser.open(os.path.realpath(MAINTENANCEFOLDER + "\\populate_dataconv_tables\\" + re.sub('\\\\\\\\', '\\\\', output_file.strip("'"))))
else:
	webbrowser.open(os.path.realpath(MAINTENANCEFOLDER + "\\populate_dataconv_tables\\dataconv_error.html"))